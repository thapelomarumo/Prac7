...
public class Parser {
	...
	public const int ListCodeOn_Sym = 55;
	public const int ListCodeOff_Sym = 56;
	public const int Warn_Sym = 57;
	...
	public const int _ListCodeOn = 55;
	public const int _ListCodeOff = 56;
	public const int _Warn = 57;
...
/* -------------------------------------------------------------------------- */
	...
	static void Statement(StackFrame frame, Label exitloop) {
		while (!(StartOf(5))) {SynErr(50); Get();}
		switch (la.kind) {
		case lbrace_Sym: {
			Block(frame, exitloop);
			break;
		}
		case const_Sym: {
			ConstDeclarations();
	...
	static void Block(StackFrame frame, Label exitblock) {
		Table.OpenScope();
		Expect(lbrace_Sym);
		while (StartOf(3)) {
	...
	static void IfStatement(StackFrame frame, Label outerBranch) {
		...
		Statement(frame, outerBranch);
		CodeGen.Branch(ifExit);
		falseLabel.Here();
		...
		if (la.kind == elsif_Sym) {
			...
			Condition();
			Expect(rparen_Sym);
			CodeGen.BranchFalse(falseLabel1);
			Statement(frame, outerBranch);
			CodeGen.Branch(ifExit);
		}
		falseLabel1.Here();
	...
	static void BreakStatement(Label exit) {
		Expect(break_Sym);
		if(exit != null){CodeGen.Branch(exit);}
		ExpectWeak(semicolon_Sym, 6);
	}
	...
	static void StringConst(out string str) {
		...
		str = token.val; str = Unescape(str.Substring(1, str.Length - 2));
		while (la.kind == stringLit_Sym || la.kind == plus_Sym) {
			if (la.kind == plus_Sym) {
				Get();
			}
			Expect(stringLit_Sym);
			string str2 = token.val;  str2 = Unescape(str2.Substring(1, str2.Length - 2)); str = str + str2;
		}
	}
	...
	static bool[,] set = {
		{T,T,x,x, x,x,x,x, x,T,x,T, T,x,x,x, x,T,T,x, x,T,x,x, T,T,T,T, T,T,T,T, T,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x},
		...
		{x,T,x,x, x,x,x,x, x,T,x,T, T,x,x,x, x,T,T,x, x,T,x,x, T,T,T,T, T,T,T,T, T,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x},
		{T,T,x,x, x,T,x,x, x,T,x,T, T,x,x,x, x,T,T,x, x,T,x,x, T,T,T,T, T,T,T,T, T,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x},
		{T,T,x,x, x,x,x,x, x,T,x,T, T,x,x,x, x,T,T,x, x,T,x,x, T,T,T,T, T,T,T,T, T,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x},
		{T,T,x,x, x,x,x,x, x,T,T,T, T,x,x,x, x,T,T,x, x,T,T,T, T,T,T,T, T,T,T,T, T,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x},
